...
open class JavaScriptView: JavaScriptClass, DisplayNodeDelegate, ScrollableDelegate, SynchronizerCallback {
	...
	open func didResolveBorders(node: DisplayNode) {
		...
		if (self.resolvedBorderTopWidth != self.node.measuredBorderTop) {
			...
			self.measuredBorderTopWidth.reset(lock: self)
		...
		if (self.resolvedBorderLeftWidth != self.node.measuredBorderLeft) {
			...
			self.measuredBorderLeftWidth.reset(lock: self)
			self.invalidateBorder()
		}
		...
		if (self.resolvedBorderRightWidth != self.node.measuredBorderRight) {
			...
			self.measuredBorderRightWidth.reset(lock: self)
			self.invalidateBorder()
		}
		...
		if (self.resolvedBorderBottomWidth != self.node.measuredBorderBottom) {
			...
			self.measuredBorderBottomWidth.reset(lock: self)
			self.invalidateBorder()
		}
	}
	 ...
	 * @hidden
	 */
	@objc open lazy var className = JavaScriptProperty(string: "") { value in
	 ...
	 * @hidden
	 */
	@objc open lazy var backgroundImagePosition = JavaScriptProperty(string: "center") { value in
	 ...
	 * @hidden
	 */
	@objc open lazy var contentDisposition = JavaScriptProperty(string: "start") { value in
	 ...
	 * @hidden
	 */
	@objc open lazy var scrollInertia = JavaScriptProperty(boolean: true) { value in
	 ...
	 * @hidden
	 */
	@objc open func jsGet_backgroundImagePosition(callback: JavaScriptGetterCallback) {
     ...
     * @hidden
     */
	@objc open func jsGet_scrollInertia(callback: JavaScriptGetterCallback) {
     ...
     * @hidden
     */
	@objc open func jsSet_scrollInertia(callback: JavaScriptSetterCallback) {
